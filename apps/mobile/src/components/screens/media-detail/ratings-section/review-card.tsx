...
export function ReviewCard({
  ...
  const hasWrittenReview = Boolean(review.title?.trim()) || Boolean(review.comment?.trim());
  ...
  return (
    <View
      ...
      {review.title?.trim() ? (
        <Text size="md" weight="bold" fillWidth numberOfLines={isPreview ? 2 : undefined}>
          {review.title.trim()}
        </Text>
      ) : null}

      {review.comment?.trim() ? (
        <Text size="sm" weight="regular" fillWidth numberOfLines={isPreview ? 4 : undefined}>
          {review.comment.trim()}
        </Text>
